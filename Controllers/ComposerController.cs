...
namespace timely_backend.Controllers {
    [ApiController]
    [Authorize(AuthenticationSchemes = "Bearer", Roles = "Administrator, Composer")]
    [Route("api/admin")]
    public class ComposerController : ControllerBase {
        ...
        public ComposerController(ILogger<ComposerController> logger, IAdminService adminService,
        ...
        public async Task<IActionResult> EditLCascadeLesson([FromBody] LessonFromId lesson, Guid id) {
            try {
                await _adminService.EditChainLesson(lesson, id);
                return Ok();
            }
            catch (ArgumentNullException e) {
                _logger.LogError(e,
